...
namespace Wow\CmsContentTranslator\Model\Catalog;
...
use Wow\CmsContentTranslator\Model\DataConverter\Renderer\Products as ProductRenderer;
 ...
 */
class ProductTotals extends \Magento\PageBuilder\Model\Catalog\ProductTotals
{
    /**
     ...
     * @return Combine
     */
    public function decodeConditions(string $conditions): Combine
    {
        if ($conditions) {
    ...
    /**
     * Retrieve count of all visible products
     *
     * @param string $conditions
     * @return int number of products not visible individually
     */
    private function getVisibleCount(string $conditions): int
    {
        $collection = $this->getProductCollection($conditions, true);
        $collection->addAttributeToFilter('status', Status::STATUS_ENABLED);
        $collection->addAttributeToFilter(
    ...
    {
        $skuList = "";
        $productCollections = $this->getProductCollection($conditions);
        foreach ($productCollections as $product) {
            $skuList!="" ? $skuList.=",".$product->getSku() : $skuList.=$product->getSku();
        }
        ...
        $visibleCount = $this->getVisibleCount($conditions);
