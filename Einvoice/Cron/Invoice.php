   ...
   /**
    ...
    public function execute()
    {
		...
		$query = "SELECT 
				...
				AND DATE_ADD(inv.created_at, INTERVAL 8 HOUR) <= DATE_ADD(NOW(), INTERVAL 8 HOUR)
				AND DATE_ADD(inv.created_at, INTERVAL 8 HOUR) >= DATE_ADD(NOW(), INTERVAL 7 HOUR)
				order by i.entity_id ASC";
				
				//AND TIMESTAMPDIFF(MINUTE,DATE_ADD(inv.created_at, INTERVAL 8 HOUR),DATE_ADD(NOW(), INTERVAL 8 HOUR)) < 300000
		
		/**
		* DATE_ADD(NOW(), INTERVAL 8 HOUR) as `current_time`,
		* TIMESTAMPDIFF(MINUTE,DATE_ADD(inv.created_at, INTERVAL 8 HOUR),DATE_ADD(NOW(), INTERVAL 8 HOUR)) 	
		*/
