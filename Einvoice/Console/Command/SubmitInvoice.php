...
class SubmitInvoice extends Command
{
    ...
    protected function configure()
    {
        ...
        $this->setName('wow:einvoice:submitinvoice');
        $this->setDescription('Submit Invoice Document');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($date = $input->getOption(self::DATE)) {
			$this->apiHelper->submitDocument($token, "invoice", $date);
		}else{
            if($incNumber = $input->getOption(self::NUMBER)){
                $this->apiHelper->submitDocument($token, "invoice", NULL, $incNumber);
            }else{
